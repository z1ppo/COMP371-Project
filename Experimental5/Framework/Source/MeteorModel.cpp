...
MeteorModel::MeteorModel(int shipTextureID,glm::vec3 size) : Model()
{
	...
	float randomX = lBound + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(uBound-lBound)));
	float randomY = lBound + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(uBound-lBound)));
	float randomZ = lBound + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(uBound-lBound)));

	mRotationAxis = vec3(randomX,randomY,randomZ);
...
void MeteorModel::Update(float dt)
{
	...
	if(this->GetPosition().z < -8.0f){
		...
		vec3 ennemyInitialPosition = vec3(ennemyPositionX, 0.0f, 70.0f);
		this->SetPosition(ennemyInitialPosition);
		//this->SetScaling(vec3(3.0f,3.0f,3.0f));
	} else if (glm::distance(mPosition, World::GetInstance()->mModel[0]->GetPosition()) < 2.007){
